...
{
    public class TestSuiteTestRunner : Game, ITestSuiteTestRunner
    {
        private readonly TestRunner runner;

        public TestSuiteTestRunner()
        {
            Add(runner = new TestRunner());
        }

        /// <summary>
        /// Blocks execution until a provided <see cref="TestSuite"/> runs to completion.
        /// </summary>
        /// <param name="test">The <see cref="TestSuite"/> to run.</param>
        public virtual void RunTestBlocking(TestSuite test) => runner.RunTestBlocking(test);
        ...
        public class TestRunner : CompositeDrawable
        {
            ...
            /// </summary>
            /// <param name="test">The <see cref="TestSuite"/> to run.</param>
            public void RunTestBlocking(TestSuite test)
            {
                ...
                void complete()
                {
                    // We want to remove the TestSuite from the hierarchy on completion as under nUnit, it may have operations run on it from a different thread.
                ...
                Schedule(() =>
                {
                    ...
                    // Nunit will run the tests in the TestSuite with the same TestSuite instance so the TestSuite
    ...
    public interface ITestSuiteTestRunner
    {
        /// <summary>
        /// Blocks execution until a provided <see cref="TestSuite"/> runs to completion.
        /// </summary>
        /// <param name="test">The <see cref="TestSuite"/> to run.</param>
        void RunTestBlocking(TestSuite test);
    }
}
