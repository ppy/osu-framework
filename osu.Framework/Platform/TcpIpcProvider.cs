                ...
                while (!token.IsCancellationRequested)
                {
                    ...
                    using (var client = await listener.AcceptTcpClientAsync())
                    {
                        using (var stream = client.GetStream())
                        {
                            ...
                            await stream.ReadAsync(header.AsMemory(0, sizeof(int)), token);
                            int len = BitConverter.ToInt32(header, 0);
                            byte[] data = new byte[len];
                            await stream.ReadAsync(data.AsMemory(0, len), token);
        ...
        public async Task SendMessageAsync(IpcMessage message)
        {
            using (var client = new TcpClient())
            {
                ...
                using (var stream = client.GetStream())
                {
                    ...
                    await stream.WriteAsync(header.AsMemory(0, header.Length));
