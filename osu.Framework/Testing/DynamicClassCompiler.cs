                    ...
                    try
                    {
                        using (var assemblyStream = File.OpenRead(t.Assembly.Location))
                        {
                            using (var peReader = new PEReader(assemblyStream))
                            {
                                ...
                                // Get the debug information for a single method from the PDB. Any method.
                                // We have to search for methods that were not generated by the compiler (i.e. those whose document is non-nil).
                                var methodDefinition = typeDefinition
                                                       .GetMethods()
                                                       .Select(pdbMetadata.GetMethodDebugInformation)
                                                       .First(m => !m.Document.IsNil);

                                // Get the document in which the method resides.
                                var document = pdbMetadata.GetDocument(methodDefinition.Document);
