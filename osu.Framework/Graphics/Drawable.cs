...
namespace osu.Framework.Graphics
{
    ...
    public abstract class Drawable : IDisposable, IDrawable
    {
        ...
        public bool TriggerHover(InputState screenSpaceState) => OnHover(cloneWithParentSpace(screenSpaceState));

        protected virtual bool OnHover(InputState state) => false;

        public void TriggerHoverLost(InputState screenSpaceState) => OnHoverLost(cloneWithParentSpace(screenSpaceState));

        protected virtual void OnHoverLost(InputState state)
        {
        }

        public bool TriggerMouseDown(InputState screenSpaceState = null, MouseDownEventArgs args = null) => OnMouseDown(cloneWithParentSpace(screenSpaceState), args);

        protected virtual bool OnMouseDown(InputState state, MouseDownEventArgs args) => false;

        public bool TriggerMouseUp(InputState screenSpaceState = null, MouseUpEventArgs args = null) => OnMouseUp(cloneWithParentSpace(screenSpaceState), args);

        protected virtual bool OnMouseUp(InputState state, MouseUpEventArgs args) => false;

        public bool TriggerClick(InputState screenSpaceState = null) => OnClick(cloneWithParentSpace(screenSpaceState));

        protected virtual bool OnClick(InputState state) => false;

        public bool TriggerDoubleClick(InputState screenSpaceState) => OnDoubleClick(cloneWithParentSpace(screenSpaceState));

        protected virtual bool OnDoubleClick(InputState state) => false;

        public bool TriggerDragStart(InputState screenSpaceState) => OnDragStart(cloneWithParentSpace(screenSpaceState));

        protected virtual bool OnDragStart(InputState state) => false;

        public bool TriggerDrag(InputState screenSpaceState) => OnDrag(cloneWithParentSpace(screenSpaceState));

        protected virtual bool OnDrag(InputState state) => false;

        public bool TriggerDragEnd(InputState screenSpaceState) => OnDragEnd(cloneWithParentSpace(screenSpaceState));

        protected virtual bool OnDragEnd(InputState state) => false;

        public bool TriggerWheel(InputState screenSpaceState) => OnWheel(cloneWithParentSpace(screenSpaceState));
        ...
        public bool TriggerFocus(InputState screenSpaceState = null, bool checkCanFocus = false)
        {
            ...
            if (checkCanFocus & !OnFocus(cloneWithParentSpace(screenSpaceState)))
                return false;
        ...
        public void TriggerFocusLost(InputState screenSpaceState = null, bool isCallback = false)
        {
            ...
            OnFocusLost(cloneWithParentSpace(screenSpaceState));
        }
        ...
        public bool TriggerKeyDown(InputState screenSpaceState, KeyDownEventArgs args) => OnKeyDown(cloneWithParentSpace(screenSpaceState), args);

        protected virtual bool OnKeyDown(InputState state, KeyDownEventArgs args) => false;

        public bool TriggerKeyUp(InputState screenSpaceState, KeyUpEventArgs args) => OnKeyUp(cloneWithParentSpace(screenSpaceState), args);

        protected virtual bool OnKeyUp(InputState state, KeyUpEventArgs args) => false;

        public bool TriggerMouseMove(InputState screenSpaceState) => OnMouseMove(cloneWithParentSpace(screenSpaceState));
        ...
        private InputState cloneWithParentSpace(InputState screenSpaceState)
        {
