...
strace -s 2000 dotnet jb inspectcode $(pwd)/osu-framework.Desktop.slnf --no-build --debug --loglevel=TRACE --logfile=$(pwd)/inspectcode.log --output=$(pwd)/inspectcodereport.xml --cachesDir=$(pwd)/inspectcode &
