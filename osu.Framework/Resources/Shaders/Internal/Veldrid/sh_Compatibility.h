...
layout(set = BASE_SET_OFFSET(set_num), binding = 0) uniform lowp TEXTURE texture_name; \
 ...
 */
#define IN(loc_num) layout(location = loc_num) in

/**
 * \brief Creates an "out" layout definition bound to the given location.
 * \param loc_num The location.
 */
#define OUT(loc_num) layout(location = loc_num) out
