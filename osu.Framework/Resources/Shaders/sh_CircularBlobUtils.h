...
highp float cro(highp vec2 a, highp vec2 b)
{
...
highp float dstToBezier(highp vec2 pos, highp vec2 A, highp vec2 B, highp vec2 C)
{
    ...
    if (h >= 0.0) // 1 root
    {
        ...
        return sqrt(dot2(q1)) * sign(cro(c + 2.0 * b * t, q1));
    }
    ...
    highp float sx = cro(c + 2.0 * b * t.x, qx);
    highp float sy = cro(c + 2.0 * b * t.y, qy);
    highp float sz = cro(c + 2.0 * b * t.z, qz);
...
lowp float blobAlphaAt(highp vec2 pixelPos, mediump float pathRadius, highp float texelSize, int pointCount, mediump float amplitude, highp vec2 noisePosition)
{
    ...
    for (int i = 0; i < pointCount; i++)
    {
        ...
        {
            highp float dstToCurve = dstToBezier(pixelPos, midLeft, currentVertex, midRight);
            highp float absDstToCurve = abs(dstToCurve);

            // save distances to current curve if it's the closest one
            dst = mix(dst, dstToCurve, float(absDstToCurve < absDst));
    ...
    return smoothstep(texelSize, 0.0, absDst + float(dst < 0.0) * (texelSize - pathRadius));
}
